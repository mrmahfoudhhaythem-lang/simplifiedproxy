...
import org.littleshoot.proxy.admin.CSVUserStore;
import org.littleshoot.proxy.admin.UserFiltersSource;
import org.littleshoot.proxy.admin.UserStoreProxyAuthenticator;
...
public class HttpFilterTest {
    ...
    @Test
    public void testAuthenticatedBlacklistedUserIsBlocked() throws Exception {
        File userFile = File.createTempFile("testUsers", ".csv");
        userFile.deleteOnExit();
        try (PrintWriter writer = new PrintWriter(new OutputStreamWriter(
                new FileOutputStream(userFile), StandardCharsets.UTF_8))) {
            writer.println("username,password,maxBandwidthBytesPerSecond,blacklistedDomains,enabled");
            writer.println("blockedUser,secret,0,localhost,true");
        }

        CSVUserStore userStore = new CSVUserStore(userFile.getAbsolutePath());
        ...
        credentialsProvider.setCredentials(
                new AuthScope("127.0.0.1", proxyServer.getListenAddress().getPort()),
                new UsernamePasswordCredentials("blockedUser", "secret"));
        ...
        try (CloseableHttpClient httpClient = HttpClients.custom()
                ...
                .setProxy(proxy)
                .build()) {
            HttpGet request = new HttpGet("http://localhost:" + webServerPort + "/");
            org.apache.http.HttpResponse response = httpClient.execute(request);
            assertEquals(403, response.getStatusLine().getStatusCode());
            HttpEntity entity = response.getEntity();
            String body = EntityUtils.toString(entity);
            assertTrue("Expected blocked response body", body.contains("Access Blocked"));
        }
    }
