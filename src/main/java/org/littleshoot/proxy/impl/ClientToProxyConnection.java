...
public class ClientToProxyConnection extends ProxyConnection<HttpRequest> {
    ...
    private ConnectionState doReadHTTPInitial(HttpRequest httpRequest) {
        ...
        // Remove proxy authorization before forwarding the request on to the target server.
    ...
    private boolean authenticationRequired(HttpRequest request) {
        ...
        // The request still retains the Proxy-Authorization header until filters
        // have a chance to inspect it. The header will be stripped later before
